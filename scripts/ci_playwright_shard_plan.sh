...
case "$MODE" in
  ...
  covoro_submit_multi)
    ...
    ;;
  covoro_ui_create_[123])
    SPEC="tests/KISHOR_UI/OMN_UIInvoice_Create_Test.spec.ts"
    PROJECT="chromium-ui"
    UI_SPEC_PART="${MODE##*_}"
    ;;
  covoro_ui_conditional_create_[123])
  ...
  covoro_ui_edit_[123])
    ...
    UI_SPEC_PART="${MODE##*_}"
    ;;
  covoro_ui_conditional_edit_[123])
  ...
  covoro_ui_copy_[123])
    ...
    UI_SPEC_PART="${MODE##*_}"
    ;;
  covoro_ui_conditional_copy_[123])
...
if [ -n "$UI_SPEC_PART" ]; then
  echo "Suite $MODE → spec $SPEC part ${UI_SPEC_PART}/3, project $PROJECT, timeout ${JOB_TIMEOUT_MINUTES}m"
else
